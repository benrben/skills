...
(function () {
  ...
  const DOC_TYPES = ["adr", "note", "rule", "rfc", "glossary"];
  ...
  function listHtml(cur) {
    ...
    let body;
    if (!total) {
      body = `<div class="docs-empty">No docs yet. Attach an ADR, note or rule to a module, a domain, or the whole system.<br><button class="btn primary docs-new" data-docs-new>+ New doc</button></div>`;
    } else if (!docs.length) {
  ...
  function reader(d) {
    ...
    const adr = d.adrRef ? `<div class="doc-adrref">ADR: <code>${esc(d.adrRef)}</code></div>` : "";
    return `
      <div class="doc-reader">
        ...
        ${adr}
        <div class="doc-body">${md(d.body)}</div>
      </div>`;
  }
  ...
  function formHtml(d) {
    ...
    return `
      <form class="doc-form" id="docForm">
        ...
        <label>ADR ref<input name="adrRef" value="${esc(d.adrRef || "")}" placeholder="docs/adr/NNNN-slug.md"></label>
